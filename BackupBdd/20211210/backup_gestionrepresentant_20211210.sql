...
/*!50001 VIEW `test` AS select `clients`.`IdClient` AS `IdClient`,`clients`.`NomClient` AS `NomClient`,`clients`.`VilleClient` AS `IdVente`,`ventes`.`IdRepres` AS `IdRepres`,`ventes`.`IdProduit` AS `IdProduit`,`ventes`.`Quantite` AS `Quantite` from (`clients` join `ventes` on((`clients`.`IdClient` = `ventes`.`IdClient`))) */;
