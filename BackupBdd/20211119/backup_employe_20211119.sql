...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `test` AS select `d`.`nodep` AS `nodep`,`d`.`nomdep` AS `nomdep`,`d`.`ville` AS `ville`,`e`.`noemp` AS `noemp`,`e`.`nomemp` AS `nomemp`,`e`.`fonction` AS `fonction`,`e`.`noresp` AS `noresp`,`e`.`datemb` AS `datemb`,`e`.`sala` AS `sala`,`e`.`comm` AS `comm` from (`employes` `e` join `departements` `d` on((`e`.`nodep` = `d`.`nodep`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
